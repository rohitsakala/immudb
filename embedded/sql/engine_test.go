...
func TestFloatType(t *testing.T) {
	...
	t.Run("must insert float type", func(t *testing.T) {
		...
		row, err := r.Read()
		require.NoError(t, err)
		require.Equal(t, Float64Type, row.Values[sel].Type())
	})
...
func TestFloatIndex(t *testing.T) {
	st, err := store.Open("float_index", store.DefaultOptions())
	require.NoError(t, err)
	defer st.Close()
	defer os.RemoveAll("float_index")
	...
	for i := 100; i > 0; i-- {
		row, err := r.Read()
		require.NoError(t, err)
		require.EqualValues(t, i, row.Values[EncodeSelector("", "db1", "float_index", "id")].Value())
	}
...
func TestFloatIndexOnNegatives(t *testing.T) {
	...
	for i := 0; i < len(floatSerie); i++ {
		row, err := r.Read()
		require.NoError(t, err)
		val := row.Values[EncodeSelector("", "db1", "float_index", "ft")].Value()
		if i == 0 {
...
func TestFloatCasts(t *testing.T) {
	st, err := store.Open("float_casts", store.DefaultOptions())
	require.NoError(t, err)
	defer st.Close()
	defer os.RemoveAll("float_casts")
	...
	for _, d := range []struct {
		...
		t.Run(fmt.Sprintf("insert a float value using a cast from '%s'", d.str), func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, Float64Type, row.Values[sel].Type())
			require.Equal(t, d.f, row.Values[sel].Value())
		})
	}
	...
	t.Run("insert a float value using a cast from INTEGER", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, Float64Type, row.Values[sel].Type())
		require.Equal(t, float64(123456), row.Values[sel].Value())
	})
