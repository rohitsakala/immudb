...
func makeDbWith(opts *Options) (DB, func()) {
	db, err := NewDB(&dummyMultidbHandler{}, opts, logger.NewSimpleLogger("immudb ", os.Stderr))
	if err != nil {
...
func TestDefaultDbCreation(t *testing.T) {
	...
	db, err := NewDB(nil, options, logger.NewSimpleLogger("immudb ", os.Stderr))
	if err != nil {
...
func TestDbCreationInAlreadyExistentDirectories(t *testing.T) {
	...
	_, err = NewDB(nil, options, logger.NewSimpleLogger("immudb ", os.Stderr))
	require.Error(t, err)
}
...
func TestOpenDB(t *testing.T) {
	...
	db, err = OpenDB(nil, options, logger.NewSimpleLogger("immudb ", os.Stderr))
	if err != nil {
...
func TestOpenV1_0_1_DB(t *testing.T) {
	...
	sysDB, err := OpenDB(nil, sysOpts, logger.NewSimpleLogger("immudb ", os.Stderr))
	require.NoError(t, err)

	dbOpts := DefaultOption().WithDBName("defaultdb").WithDBRootPath("./data_v1.1.0")
	db, err := OpenDB(nil, dbOpts, logger.NewSimpleLogger("immudb ", os.Stderr))
	require.NoError(t, err)
